...
class GraspGenerator:
    ...
    def __init__(self, net_path, camera, depth_radius, fig, IMG_WIDTH=224, network='GR_ConvNet', device='cpu'):
        ...
        if (device=='cpu'):
            ...
            elif (network == 'GG_CNN' or network == 'GG2'):
                self.net = torch.load(net_path, map_location=device)
                # self.net = GGCNN()
    ...
    def predict(self, rgb, depth, bbox, mask, n_grasps=1, show_output=False):
        ...
        elif (self.network == 'GG_CNN' or self.network == 'GG2'):
            ##### GG_CNN #####

            x = torch.from_numpy(depth.reshape(1, 1, self.IMG_WIDTH, self.IMG_WIDTH).astype(np.float32))

            # depth = np.expand_dims(np.array(depth), axis=2)
            # # img_data = CameraData(width=self.IMG_WIDTH, height=self.IMG_WIDTH, output_size=300, include_rgb=False)
            # img_data = CameraData(width=self.IMG_WIDTH, height=self.IMG_WIDTH, include_rgb=False)
            # x, depth_img, rgb_img = img_data.get_data(depth=depth)
